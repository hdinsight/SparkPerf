...
class ImpalaKitQueries(
  ...
  lazy val queries = Seq(
    ...
    case (name, sqlText) => Query(sparkSession, name, sqlText,
  ...
  lazy val queriesMap = queries.map(q => q.name -> q).toMap

  private lazy val interactiveQueries =
    Seq("q19", "q42", "q52", "q55", "q63", "q68", "q73", "q98").map(queriesMap)
  private lazy val reportingQueries = Seq("q3", "q7", "q27", "q43", "q53", "q89").map(queriesMap)
  private lazy val deepAnalyticQueries =
    Seq("q34", "q46", "q59", "q65", "q79", "ss_max").map(queriesMap)
