...
abstract class Benchmark(
    ...
    tableRootpath: Option[String]) extends Serializable {

  import Benchmark._

  def this(sqlContext: SQLContext) = this(sqlContext, None)

  def this(tableRootPath: String) = this(SQLContext.getOrCreate(SparkContext.getOrCreate()),
    Some(tableRootPath))
  ...
  protected def buildTables(): Unit = {}
  ...
  def runExperiment(
    ...
    println(s"allTables: ${allTables.map(_.name)}")
    new ExperimentStatus(executionsToRun, includeBreakdown, iterations, variations, tags,
      ...
      forkThread = forkThread, reportFormat)
  }
  ...
  class SparkPerfExecution(
      ...
      prepare: () => Unit,
      run: () => Unit,
      description: String = "") extends Benchmarkable {
